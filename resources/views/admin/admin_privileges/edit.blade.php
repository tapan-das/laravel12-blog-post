...
<div class="row">
	<div class="col-md-6 offset-md-3">
		<div class="card">
            ...
            <div class="card-body">
            <form action="{{ route('postUpdatePrivilege', $row->id) }}" method="post" enctype="multipart/form-data"> 
        	@csrf
            ...
            <div id="privileges_configuration" class="mb-3">
                ...
                <table class="table table-striped table-hover table-bordered">
                    ...
                    <tbody>
                        ...
                        <?php
                        $roles = \App\Models\AdminPrivilegeRole::where('id_admin_menus', $menu->id)->where('id_admin_privileges', $row->id)->first();
                        ?>
                        <tr>                            
                            <td>{{ $menu->name }}</td>
                            ...
                            <td align="center">
                                <input type="checkbox" class="is_visible" name="privileges[{{$menu->id}}][is_visible]"  <?=@$roles->is_visible ? "checked" : ""?> value="1">
                            </td>
                            <td class="warning" align="center">
                                <input type="checkbox" class="is_create" name="privileges[{{$menu->id}}][is_create]" <?=@$roles->is_create ? "checked" : ""?> value="1">
                            </td>
                            <td class="info" align="center">
                                <input type="checkbox" class="is_read" name="privileges[{{$menu->id}}][is_read]" <?=@$roles->is_read ? "checked" : ""?> value="1">
                            </td>
                            <td class="success" align="center">
                                <input type="checkbox" class="is_edit" name="privileges[{{$menu->id}}][is_edit]" <?=@$roles->is_edit ? "checked" : ""?> value="1">
                            </td>
                            <td class="danger" align="center">
                                <input type="checkbox" class="is_delete" name="privileges[{{$menu->id}}][is_delete]" <?=@$roles->is_delete ? "checked" : ""?> value="1">
                            </td>
                        </tr>
                        @endforeach
                        @else
